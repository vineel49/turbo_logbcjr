...
function [LLR]= log_BCJR_END(LLR,My_Gamma,num_bit)
...
%******************************************************************************
 for time=1:num_bit-1
     % forward recursion
     temp1 = log_alpha(Prev_State(:,1),time)+log(repmat_C(time))+(1-2*(Prev_Ip(:,1)-1))*LLR(time)/2+My_Gamma(Outputs_prev(:,1),time);
     temp2 = log_alpha(Prev_State(:,2),time)+log(repmat_C(time))+(1-2*(Prev_Ip(:,2)-1))*LLR(time)/2+My_Gamma(Outputs_prev(:,2),time);
     log_alpha(:,time+1)= max(temp1,temp2)+log(1+exp(-abs(temp1-temp2))) ; % Jacobian logarithm
     % backward recursion
     temp3 = log_beta(Next_State(:,1),num_bit+2-time)+log(repmat_C(num_bit+1-time))+(1-2*(Next_Ip(:,1)-1))*LLR(num_bit+1-time)/2+My_Gamma(Outputs_next(:,1),num_bit+1-time);
     temp4 = log_beta(Next_State(:,2),num_bit+2-time)+log(repmat_C(num_bit+1-time))+(1-2*(Next_Ip(:,2)-1))*LLR(num_bit+1-time)/2+My_Gamma(Outputs_next(:,2),num_bit+1-time);
...
%**************************************************************************
 temp5 = log_alpha+repmat(LLR/2,num_states,1)+My_Gamma(Outputs_next(:,1),:)+log_beta(Next_State(:,1),2:num_bit+1) ;
 ...
 temp6 = log_alpha-repmat(LLR/2,num_states,1)+My_Gamma(Outputs_next(:,2),:)+log_beta(Next_State(:,2),2:num_bit+1) ;
